...
const columns: Column[] = [
  ...
  { id: 'inprogress', title: 'In Progress', icon: ArrowRight, color: '#17a2b8' },
...
const initialTasks: Task[] = [
  {
    ...
    description: 'Create wireframes and mockups for the login and registration pages',
    priority: 'high',
    assignee: 'John Doe',
    dueDate: '2024-01-15',
  },
  ...
  return (
    ...
    >
      <Card.Body className='p-3'>
        ...
        <Card.Text className='small text-muted mb-2'>{task.description}</Card.Text>
        <Stack direction='horizontal' gap={2} className='mb-2'>
          <Badge bg={getPriorityVariant(task.priority)}>{task.priority}</Badge>
          {task.assignee && <small className='text-muted'>Assignee: {task.assignee}</small>}
        </Stack>
        {task.dueDate && (
...
export default function KanbanExamplePage() {
  ...
  const tasksByColumn = columns.map((column) => {
    const columnTasks = tasks.filter((task) => {
      ...
      if (column.id === 'done') return task.id === 'task-3' || task.id === 'task-5';
  ...
  return (
    <div className='py-4  min-vh-100'>
      <Container fluid>
        <h1 className='mb-4'>Kanban Board Example</h1>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <Row>
            {tasksByColumn.map(({ column, tasks }) => {
              const Icon = column.icon;
              return (
                <Col key={column.id} md={2} className='mb-4'>
                  <Card className='border-0 shadow-sm h-100'>
                    ...
                    >
                      <div className='d-flex align-items-center gap-2'>
                        ...
                        <Card.Title className='h6 mb-0 fw-semibold' style={{ color: column.color }}>
                          {column.title}
                        </Card.Title>
                      </div>
                    ...
                    <Card.Body className='p-2'>
                      ...
                      <Button variant='outline-secondary' className='w-100 mt-2' size='sm'>
