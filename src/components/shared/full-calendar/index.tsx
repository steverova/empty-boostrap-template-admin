...
export default function FullCalendar({
  ...
  const isEventStart = (event: CalendarEvent, date: Date) => isSameDay(event.date, date)
  ...
  const headerTitle = () => {
    if (currentView === 'month') return format(currentDate, 'MMMM yyyy', { locale })
    if (currentView === 'week')
  ...
  const getDayClasses = (day: Date, isCurrentMonth: boolean, isToday: boolean) => {
    ...
    if (selectedRange) {
      ...
      else if (isDateInRange(day, selectedRange)) classes.push(calendarDayInRange)
    }
    return classes.join(' ')
  }

  const rangeLabel = selectedRange
    ? `${format(selectedRange.start < selectedRange.end ? selectedRange.start : selectedRange.end, 'd MMM')} – ${format(selectedRange.start < selectedRange.end ? selectedRange.end : selectedRange.start, 'd MMM yyyy')}`
    : null

  if (currentView === 'day') {
    const dayEvents = getEventsForDate(currentDate)
    return (
      <div className={`${calendarContainer} bg-success`}>
        <div className={calendarHeader}>
          <div className={calendarNav}>
            <Button variant="outline-secondary" size="sm" onClick={navigatePrev}>
              <ChevronLeft size={16} />
            </Button>
            <h5 className="mb-0 mx-3">{headerTitle()}</h5>
            <Button variant="outline-secondary" size="sm" onClick={navigateNext}>
              <ChevronRight size={16} />
            </Button>
            <Button variant="outline-secondary" size="sm" onClick={navigateToday} className="ms-2">
              Hoy
            </Button>
          </div>
          <ButtonGroup size="sm">
            {(['month', 'week', 'day'] as ViewMode[]).map((v) => (
              <Button
                key={v}
                variant={currentView === v ? 'primary' : 'outline-secondary'}
                className={currentView === v ? viewButtonActive : viewButton}
                onClick={() => setView(v)}
              >
  ...
  return (
    <div className={calendarContainer}>
      ...
      <div className={calendarGrid}>
        <div className={`${calendarDayHeader} d-grid`} style={{ gridTemplateColumns: 'repeat(7, 1fr)' }}>
          {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map((d) => (
            <div key={d} className="text-center py-2 fw-medium text-uppercase small">
              {d}
            </div>
          ))}
        </div>
        ...
        {weeks.map((week, wi) => (
          <div key={wi} className="d-grid" style={{ gridTemplateColumns: 'repeat(7, 1fr)' }}>
            {week.map((day) => {
                ...
                >
                  ...
                  <div className="d-flex flex-column gap-1">
                    {dayEvents.slice(0, 3).map((event) => {
                      ...
                      const pillClass = isStart ? eventPill : isEnd ? eventPillEnd : eventPillContinuation
                      return (
                        <div
                          key={event.id}
                          className={pillClass}
